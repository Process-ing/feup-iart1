...
class Controller:
    ...
    def process_command(self, tokens: list[str]) -> CommandResult:
        ...
        if command in ["solve"]:
            ...
            algorithm_name = None if len(tokens) == 1 else tokens[1]
            if algorithm_name == "random-walk":
                max_iterations = 200 if len(tokens) < 2 else int(tokens[2])
                ...
                temperature = 100000 if len(tokens) < 2 else int(tokens[2])
                cooling_schedule = 0.99 if len(tokens) < 3 else float(tokens[3])
                max_iterations = 200 if len(tokens) < 4 else int(tokens[4])
                ...
                tabu_tenure = None if len(tokens) < 2 else int(tokens[2])
                neighborhood_len = 10 if len(tokens) < 3 else int(tokens[3])
                max_iterations = None if len(tokens) < 4 else int(tokens[4])
                ...
                population_size = 10 if len(tokens) < 2 else int(tokens[2])
                max_generations = 1000 if len(tokens) < 3 else int(tokens[3])
                algorithm = GeneticAlgorithm(problem, population_size=population_size, max_generations=max_generations)
            elif algorithm_name in ["--help", "-h"]:
                print_solve_usage()
                return CommandResult.SUCCESS
            else:
                print_solve_usage()
